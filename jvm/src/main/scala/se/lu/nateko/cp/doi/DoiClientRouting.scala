...
class DoiClientRouting(
	clients: Map[String, DoiClient],
	defaultEnv: String,
	conf: DoiConfig
)(using ActorSystem) {
	import DoiClientRouting._

	private def withClient = parameters("env".optional).flatMap{ envOpt =>
		val envName = envOpt.getOrElse(defaultEnv)
		clients.get(envName) match
			case Some(client) => provide(client)
			case None => complete(StatusCodes.BadRequest -> s"Unknown environment: $envName")
	}
	...
	def writingRoute(authorizer: (DoiMeta, DoiClient) => Future[Boolean])(using ExecutionContext) = post{
		path("metadata"){
			withClient{ client =>
				entity(as[DoiMeta]){meta =>
					onSuccess(authorizer(meta, client)){allowed =>
						if(allowed)
	...
	def deleteRoute(using ExecutionContext) = withClient{ client =>
